...
function stopTimer() {
    ...
    // Auto-next banner
    if (currentAgendaIndex !== null) {
        const nextIndex = currentAgendaIndex + 1;
        if (scannedAgendaFlow[nextIndex]) {
            const nextItem = scannedAgendaFlow[nextIndex];
            ...
            banner.innerHTML = `
                Next: <strong>${nextItem.role}</strong>
                ${nextItem.name ? " – " + nextItem.name : ""}
                &nbsp;&nbsp;
                <button id="autoNextStartBtn" style="
                    background:white;
                    color:#2196F3;
                    border:none;
                    padding:6px 12px;
            ...
            document.getElementById("autoNextStartBtn").onclick = () => {
                document.body.removeChild(banner);
                startFromAgendaItem(nextIndex);
            };
            ...
            logger: m => {
                if (m.status === "recognizing text") {
                    scanStatusEl.textContent =
                        `Scanning image… ${Math.round(m.progress * 100)}%`;
                }
            }
        });
...
/* ============================
   PARSE AGENDA TEXT (strong 3-column)
============================ */
function parseAgendaText(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (parts.length >= 3) {
            ...
            const rolePart = parts[0].replace(timeRegex, "").trim();
    ...
    if (flow.length === 0) {
        ...
        if (missingDur.length > 0) {
            scanStatusEl.textContent =
                `Agenda parsed with ${flow.length} items. Some items missing durations – preset timings will be used and time alerts shown.`;
        } else {
            scanStatusEl.textContent =
                `Agenda parsed with ${flow.length} items. All items have durations or times.`;
        }
    }
...
function renderScannedFlow() {
    if (!scannedAgendaFlow || scannedAgendaFlow.length === 0) {
        scanFlowContainer.innerHTML =
            "<p style='font-size:12px;opacity:0.8;'>No scanned agenda yet.</p>";
        return;
    }
    ...
    scannedAgendaFlow.forEach((item, index) => {
        const durationText = item.duration
            ? item.duration
            : "No duration – will use preset timing";
        const alertText = item.duration
            ? ""
            : "<span style='color:#FFC107;'>⚠ Missing time – preset timing + alert</span>";
...
function applyScannedAgendaToMeeting() {
    ...
    scanStatusEl.textContent =
        "Scanned agenda applied to this meeting. You can start items from the Scan Agenda tab or use manual Start.";
    saveMeetingToCookie();
}
